class HitboxEditor extends GameWorldWindow {
  ...
  select(object) {
    if(!object) return
    this.deselect()
    // GameObject.remove_from_stage(object)
    // GameObject.add_to_stage()
  ...
  deselect() {
    if(!this.obj) return
    editor_app.stage.removeChild(this.obj.container)
  ...
  handle_keydown(event) {
    ...
    if(document.activeElement !== this.search_bar.input) { 
      ...
      if(event.code === "Escape") {
        ...
        if(this.state.is("defining_weapon_slots")) {
          this.set_tool("select")
  ...
  handle_mousedown(event) {
    if(this.obj && event.target === this.canvas) {
      ...
      if(this.state.is("defining_weapon_slots")) {
        let newslot = mouse.editor_pos.clone()
        this.weapon_slots.push(newslot)
      }
  ...
  handle_click(event) {
    ...
    if(target.closest(".tool-cont")) {
      ...
      if(el.dataset.toolname === "define_weapon_slots") {
        this.weapon_slots = []
        this.state.set("defining_weapon_slots")
  ...
  get_clicked_handle() {
    ...
    if(smallest < this.control_point_radius) {
  ...
  draw_control_points() {
    ...
    control_points.forEach(point => {
      graphics.drawCircle(point.x, point.y, this.control_point_radius)
    })
  }
  draw_weapon_slots() {
    this.weapon_slots.forEach(slot => {
      this.graphics.lineStyle(2, colors.hitbox.weapon_slot, 1)
      this.graphics.drawCircle(slot.x, slot.y, 9)
      this.graphics.closePath()
    })
  }
  draw_unfinished_shape() {
    Hitbox.draw_polygon(this.shape, this.graphics)
  }
  update() {
    this.generate_control_points()
    this.draw_weapon_slots() 
