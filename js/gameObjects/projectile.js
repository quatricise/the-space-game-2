class Projectile extends Rigid {
  constructor(
    ...
    this.impactDamage = projectile.impactDamage
  ...
  blackhole() {
    ...
    objs.forEach(obj => {
      ...
      let strength = 120
      let vec = Vector.fromAngle(angle).mult(strength).mult(dist / maxDist).mult(dt)
      if(dist < maxDist) {
        obj.vel.add(vec)
      }
    })
